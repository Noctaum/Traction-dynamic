...
function considerAllWeight() {
    var considerAllWeight01 = (get("weightCar")+(get("quantityPasanger")+get("carrying")+1)*(75+10))/1000;
...
function transmission() {
    ...
    if(document.getElementById("transmissionWheelDrive").checked){
        (document.getElementById("payt1").checked) ? transmission01*=Math.pow(get("transmissionWheel"),get("transmissionQuantitywheelDrive")):transmission01*=Math.pow(0.97,get("transmissionQuantitywheelDrive"));
    }
...
function tires() {
    ...
    var halfWeigth1 = document.getElementsByName('halfWeigth1');
    var m2,koeff;
    switch(true){
        case(halfWeigth1[0].checked):
            koeff=0.68;
            break;
        case(halfWeigth1[1].checked):
            koeff=0.72;
            break;
    }
    switch(true){
        case(halfWeigth[0].checked):
            m2 = koeff*sborochka.mA;
...
function topGear(a,b,c,vSp,puf1,puf2) {
    ...
    if(c==99){switch (true){
        ...
        case (tiresMass.driveUnit==1):{Krf=0.9; break;}
        case (tiresMass.driveUnit==2):{Krf=1.1; break;}
    }}
    else{switch (true){
        case (tiresMass.driveUnit==0):{Krf=1; break;}
        case (tiresMass.driveUnit==1):{Krf=0.87; break;}
        case (tiresMass.driveUnit==2):{Krf=1.07; break;}
    }}
    ...
    if(!document.getElementById("payt2").checked){
        ...
        if (U1f>U1ff && puf2==0){topGear(0.35,0.85,99,7,1,1);return;}
    ...
    if (U1v<=U1ff && U1v>=U1f ) tiresMass.U1=U1v.toFixed(3);
        ...
        for (var i=U1ff-0.02; i>=U1f-0.1; i-=0.01){
            ...
            if(predU>=2 && predU<=7 ) {tiresMass.U1=i.toFixed(3); break;}
...
function tireNext(puf0, puf3, puf4) {
    ...
    if(predU<2 || predU>7) alert("Warning! Vamin выходит за пределы 2..7 км/ч.");
    document.getElementById("firstGear2").innerHTML = (+tiresMass.U1).toFixed(3);
    document.getElementById("firstGear").innerHTML = (+tiresMass.U1).toFixed(3);
    document.getElementById("topGearEnd").value =tiresMass.U1;
...
function tractionCharacteristic() {
    ...
    leftCanv(0,Ftmax + Ftmax/10,500,1,5000,1000,sch,Vmax+20);
...
function economy() {
    document.getElementById("spechialSpeed").innerHTML=(sborochka.Vemax*3/4).toFixed(3);
     var V,P=0;
     for (var i=0; i<+stairs.quantity; i++){
         if (P<massP[i]){
             P=massP[i];
    ...
    var GEP=1.05, GEM=180, fuelDensity=840;
    ...
    for(i=0; i<+stairs.quantity; i++){
        ...
        Ki[i]=(1.2+0.14*U[i]-1.8*U[i]*U[i]+1.46*U[i]*U[i]*U[i]).toFixed(3);
